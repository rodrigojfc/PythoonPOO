...
Cargo = StringVar()
Salario = StringVar()
Fechadenacimiento = StringVar()
TipoContratacion = StringVar()
...
def crear():
    ...
    miCursor = miConexion.cursor()
    try:
        datos = Nombre.get(), Cargo.get(), Salario.get(), Fechadenacimiento.get(), TipoContratacion.get()
...
def actualizar():
    ...
    miCursor = miConexion.cursor()
    try:
        datos = Nombre.get(), Cargo.get(), Salario.get(), Fechadenacimiento.get()
        miCursor.execute("UPDATE empleado SET Nombre=?, Cargo=?, Salario=?, Fechadenacimiento=?, TipoContratacion=? WHERE Id="+ Id.get(), (datos))
        miConexion.commit()
    except:
...
Tree = ttk.Treeview(height = 10, columns = ('#0', '#1', '#2', '#4', '#5'))
...
Tree.heading('#4', text="Fecha de nacimiento", anchor = CENTER)
Tree.heading('#5', text= "Tipo de contrato", anchor = CENTER)
...
def seleccionarUsandoClick(event):
    ...
    Fechadenacimiento.set(Tree.item(item, "values")[4])
    TipoContratacion.set(Tree.item(item, "values")[5])
...
'''La siguiente variable creada tendra el Id de cada registro, no se mostrara es decir sera una variable oculta que 
...
l6 = Label(ventana, text="Fecha de nacimiento")
l6.place(x=440, y=40)
e6 = Entry(ventana, textvariable= Fechadenacimiento, width=10 )
e6.place(x=560, y=40)

l7 = Label(ventana, text="Tipo de contratacion")
l7.place(x=660, y=40)
e7= Entry(ventana, textvariable= TipoContratacion, width=10)
e7.place(x=780, y=40)
